...
export default function EnhancedMap({
  ...
  heistandConfidence = { high: true, medium: true, low: true },
  pinsGeoJSON,
  pinDropMode = false,
  ...
  // Flipped once the 'load' handler has finished adding sources/layers. Overlay
  // toggles must depend on this: a prop-only effect can fire before the layers
  // exist, silently no-op, and never re-run.
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      // Heistand family farm portfolio (generated by scripts/build-heistand-portfolio.mts)
      map.current!.addSource('heistand-farms', {
        type: 'geojson',
        data: '/heistand-farms.geojson'
      });

      map.current!.addLayer({
        id: 'heistand-farms-fill',
        type: 'fill',
        source: 'heistand-farms',
        layout: { visibility: showHeistandFarms ? 'visible' : 'none' },
        paint: {
          // Colour encodes how well the farm matched the assessor parcel data,
          // so a guessed match never looks as authoritative as a confirmed one.
          'fill-color': [
      ...
      map.current!.addLayer({
        id: 'heistand-farms-labels',
        type: 'symbol',
        source: 'heistand-farms',
        minzoom: 9,
        layout: {
          visibility: showHeistandFarms ? 'visible' : 'none',
          'text-field': ['get', 'farm'],
          'text-size': 12,
          'text-anchor': 'center',
          'text-allow-overlap': false
      ...
      // Farm detail popup on click
      map.current!.on('click', 'heistand-farms-fill', (e) => {
        const f = e.features?.[0];
        if (!f) return;
        const p = f.properties as Record<string, any>;
        const money = (v: any) =>
          v == null || v === '' ? null : `$${Number(v).toLocaleString('en-US', { maximumFractionDigits: 0 })}`;
        const row = (label: string, value: any) =>
          value == null || value === '' ? '' : `<div style="display:flex;justify-content:space-between;gap:12px"><span style="color:#64748b">${label}</span><span style="font-weight:500">${value}</span></div>`;
        const badge = { high: ['#065f46', '#d1fae5', 'Confirmed'], medium: ['#92400e', '#fef3c7', 'Likely'] }[p.confidence as string]
          ?? ['#9f1239', '#ffe4e6', 'Needs review'];

        new maplibregl.Popup({ closeButton: true, maxWidth: '320px' })
          .setLngLat(e.lngLat)
          .setHTML(`
            <div style="font-family:system-ui,sans-serif;font-size:12px;line-height:1.6">
              <div style="font-weight:600;font-size:14px;margin-bottom:2px">${p.farm ?? ''}</div>
              ${p.location ? `<div style="color:#64748b;margin-bottom:6px">${p.location}</div>` : ''}
              <span style="display:inline-block;padding:1px 8px;border-radius:9999px;font-size:10px;font-weight:600;color:${badge[0]};background:${badge[1]};margin-bottom:8px">${badge[2]}</span>
              ${row('Owner', p.owner)}
              ${row('County', p.county)}
              ${row('Tax acres', p.taxAcres)}
              ${row('FSA acres', p.fsaAcres)}
              ${row('CSR', p.csr)}
              ${row('Rent / acre', money(p.rentPerAcre))}
              ${row('Sale price', money(p.salePrice))}
              ${row('Tenant', p.tenant)}
              <hr style="border:none;border-top:1px solid #e2e8f0;margin:8px 0" />
              ${row('Parcels drawn', `${p.parcelCount} (${p.dbAcres} ac)`)}
              ${row('Titled to', p.matchedDeedHolder)}
            </div>
          `)
          .addTo(map.current!);
      });

      map.current!.on('mouseenter', 'heistand-farms-fill', () => {
        if (map.current) map.current.getCanvas().style.cursor = 'pointer';
      });
      map.current!.on('mouseleave', 'heistand-farms-fill', () => {
        if (map.current) map.current.getCanvas().style.cursor = '';
      });
  ...
  // Toggle the Heistand portfolio and filter it by match confidence
  useEffect(() => {
    if (!map.current) return;

    const enabled = (['high', 'medium', 'low'] as const).filter(k => heistandConfidence[k]);
    // An empty selection would filter to nothing; hide the layers instead so the
    // checkbox state stays honest rather than showing a silently empty map.
    const visible = showHeistandFarms && enabled.length > 0;
    const filter = ['in', ['get', 'confidence'], ['literal', enabled]] as any;

    ['heistand-farms-fill', 'heistand-farms-outline', 'heistand-farms-labels'].forEach(layerId => {
      if (!map.current?.getLayer(layerId)) return;
      map.current.setLayoutProperty(layerId, 'visibility', visible ? 'visible' : 'none');
      map.current.setFilter(layerId, filter);
    });
  }, [showHeistandFarms, heistandConfidence, mapReady]);
