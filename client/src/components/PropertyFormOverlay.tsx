...
export default function PropertyFormOverlay({ onClose, onValuationCreated, drawnPolygonData, parcelData }: PropertyFormOverlayProps) {
  ...
  const fetchParcelCSR2Data = async () => {
    ...
    try {
      ...
      // Try enhanced polygon endpoint first if we have geometry
      if (polygons.length > 0) {
        try {
          // Convert first polygon to WKT for enhanced analysis
          const polygon = polygons[0];
          const coords = polygon.geometry.coordinates[0];
          const wktCoords = coords.map((c: [number, number]) => `${c[0]} ${c[1]}`).join(', ');
          const wkt = `POLYGON((${wktCoords}))`;
          
          toast({
            title: "Analyzing Soil Quality",
            description: `Performing comprehensive soil analysis for ${Math.round(parcelData.acres)} acre parcel.`,
            variant: "default",
          });
          ...
          if (data.success && data.data) {
            ...
            const resultData = {
              ...
              csr2: {
                success: true,
                mean: enhancedData.summary.weightedAverageCSR2,
                min: enhancedData.statistics.csr2.min,
      ...
      // Fallback: Original point sampling method
    ...
    >
      ...
      >
        ...
        <div className="p-4 sm:p-6 overflow-y-auto h-[calc(100%-4rem)] sm:h-[calc(100%-5rem)]">
          {/* Enhanced Soil Properties Display */}
          {parcelCSR2Data?.enhanced && (
              ...
              <div className="bg-gradient-to-r from-emerald-50 to-blue-50 p-4 rounded-lg border border-emerald-200">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-white p-3 rounded border border-slate-200">
                    <div className="text-xs text-slate-500">CSR2 Rating</div>
                    <div className="text-lg font-semibold text-emerald-700">
                      {parcelCSR2Data.enhanced.summary.weightedAverageCSR2.toFixed(1)}
                    </div>
                    <div className="text-xs text-slate-400">
                      Range: {parcelCSR2Data.enhanced.statistics.csr2.min} - {parcelCSR2Data.enhanced.statistics.csr2.max}
                    </div>
                  </div>
                  
                  {parcelCSR2Data.enhanced.summary.averageSlope !== undefined && (
                    <div className="bg-white p-3 rounded border border-slate-200">
                      <div className="text-xs text-slate-500">Average Slope</div>
                      <div className="text-lg font-semibold text-blue-700">
                        {parcelCSR2Data.enhanced.summary.averageSlope.toFixed(1)}%
                      </div>
                      {parcelCSR2Data.enhanced.statistics.slope && (
                        <div className="text-xs text-slate-400">
                          Range: {parcelCSR2Data.enhanced.statistics.slope.min}% - {parcelCSR2Data.enhanced.statistics.slope.max}%
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="bg-white p-3 rounded border border-slate-200">
                    ...
                    <div className="text-lg font-semibold text-slate-700">
                      {parcelCSR2Data.enhanced.summary.totalAcres}
                    </div>
                  </div>
                  
                  <div className="bg-white p-3 rounded border border-slate-200">
                    <div className="text-xs text-slate-500">Soil Units</div>
                    <div className="text-lg font-semibold text-slate-700">
                      {parcelCSR2Data.enhanced.summary.soilUnitCount}
                    </div>
                  </div>
                </div>
                
                {parcelCSR2Data.enhanced.summary.drainageClass && (
                  <div className="bg-white p-2 rounded border border-slate-200 text-xs">
                    <span className="text-slate-500">Drainage: </span>
                    <span className="font-medium text-slate-700">{parcelCSR2Data.enhanced.summary.drainageClass}</span>
                  </div>
                )}
              </div>
              
              {/* Soil Composition Table */}
              {parcelCSR2Data.enhanced.soilComposition && parcelCSR2Data.enhanced.soilComposition.length > 0 && (
                <details className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                  <summary className="cursor-pointer text-sm font-semibold text-slate-700 mb-2">
                    Soil Composition ({parcelCSR2Data.enhanced.soilComposition.length} soil types)
                  </summary>
                  <div className="mt-3 space-y-2">
                    {parcelCSR2Data.enhanced.soilComposition.slice(0, 5).map((soil, idx) => (
                      <div key={soil.mukey} className="bg-white p-2 rounded border border-slate-200">
                        <div className="flex justify-between items-start mb-1">
                          <div className="text-xs font-medium text-slate-700 flex-1">
                            {soil.name}
                          </div>
                          <div className="text-xs font-semibold text-emerald-600">
                            {soil.percentage.toFixed(1)}%
                          </div>
                        </div>
                        <div className="flex gap-3 text-xs text-slate-500">
                          <span>CSR2: {soil.csr2}</span>
                          {soil.slope !== undefined && <span>Slope: {soil.slope}%</span>}
                          {soil.cornYield && <span>Corn: {soil.cornYield} bu/ac</span>}
                        </div>
                      </div>
                    ))}
                  </div>
