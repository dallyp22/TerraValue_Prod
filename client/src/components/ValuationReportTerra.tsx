...
export function ValuationReportTerra({ valuation, className = '' }: ValuationReportTerraProps) {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      pdf.text('TerraValue Property Valuation Report', margin, margin);
      ...
      const filename = `TerraValue_${valuation.address?.replace(/[^a-zA-Z0-9]/g, '_') || 'Property'}_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(filename);
    } catch (error) {
    ...
    >
      ...
      <div className="px-6 py-4 bg-warm-50 border-t border-warm-200">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-warm-500">
            Generated by TerraValue on {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
